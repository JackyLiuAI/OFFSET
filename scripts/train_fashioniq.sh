...
# Set JOINT=1 to train dress+shirt+toptee jointly with unified weights
JOINT=${JOINT:-1}
...
# Joint training for dress + shirt + toptee (unified weights)
run_joint_train() {
  echo "[INFO] Starting JOINT training for categories: dress, shirt, toptee"

  python -u "${PROJECT_ROOT}/train.py" \
    --local_rank "${LOCAL_RANK}" \
    --dataset "fashioniq" \
    --multi_datasets dress shirt toptee \
    --fashioniq_split "${FASHIONIQ_SPLIT}" \
    --fashioniq_path "${FASHIONIQ_ROOT}" \
    --shoes_path "${SHOES_PATH}" \
    --cirr_path "${CIRR_PATH}" \
...
# Entry: single or joint
if [ "${JOINT}" = "1" ]; then
  run_joint_train
else
  # Train single category; set CATEGORY env var to one of dress/shirt/toptee
  run_train "${CATEGORY}"
fi
